...
class MarkersControl(metaclass=Singleton):
    ...
    def FindTarget(self) -> Optional[Marker]:
        """
    ...
    def FindPointOfInterest(self) -> Optional[Marker]:
        for marker in self.list:
            if marker.is_point_of_interest:
                return marker
    ...
    def ChangeColor(self, marker: Marker, new_color: List[int]) -> None:
        """
    ...
    def DeleteBrainTargets(self) -> None:
        def find_brain_target() -> Optional[Marker]:
            for marker in self.list:
